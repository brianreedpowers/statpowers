...
var colors = ["red","blue","limegreen","yellow","purple","darkorange","dimgray","lightpink","dodgerblue","springgreen","greenyellow","thistle","navajowhite","ivory","crimson","darkblue","darkgreen","olive","blueviolet","sienna","black","maroon","paleturquoise","darkcyan","darkseagreen","mediumslateblue","rosybrown","darkslateblue"]
var colorsFont = ['black','white','black','black','white','black','black','black','black','black','black','black','black','black','white','white','black','black','black','white','black','black','black','black','black','white']
var colorHexes = ["#ff0000","#0000ff","#00ff00","#ffff00","#800080","#ff8c00","#696969","#fa8072","#6495ed","#00ff7f","#adff2f","#d8bfd8","#ffdead","#fffff0","#dc143c","#00008b","#006400","#808000","#8a2be2","#a0522d","#000000","#b03060","#afeeee","#008b8b","#8fbc8f","#7b68ee","#bc8f8f","#483d8b"]
...
function fiveNumSum(data){
	...
	if(n<=3){
		fiveNumSumArray[1]=estimatePercentile(data, .25)
		fiveNumSumArray[2]=estimatePercentile(data, .5)
		fiveNumSumArray[3]=estimatePercentile(data, .75)
	}
...
function cleanDataInput(nSets){
	for(var set=1; set<=nSets; set++){
		var dataString = document.getElementById("data"+set).value.trim()
		dataString=dataString.replaceAll('–','-')
		dataString=dataString.replaceAll('—','-')

		var data = dataString.split(/[\s,;\t\n]+/)
		var n=data.length
		var myArray = [];
		for (var i = 0; i<n; i++){
...
function arraySample(x, n, replace=false){
	if(replace==false){
		return shuffle(x).splice(n)
	} else {
		var samp=[]
		var xn=x.length
		for(var i=0; i<n; i++){
			var j=Math.floor(Math.min(xn-1,Math.random()*xn))
			samp.push(x[j])
		}
		return samp
	}
...
function parseDataFromURL(nSets){
	for(var i=1; i<=nSets; i++){
		...
		if(exists('nIndepSamplesSelect')) document.getElementById('nIndepSamplesSelect').value=nIndepSamples
	...
	if(getQueryVariable("K")!=null){
		document.getElementById("nIndepSamplesSelect").value = parseInt(decode(getQueryVariable("K")));
		nIndepSamples =parseInt(decode(getQueryVariable("K")))
	}
}
...
function compressData(dataString, times="x"){
	...
	var data = dataString.trim().match(/\\?.|^$/g).reduce((p, c) => {
					if(c === '"'){
						p.quote ^= 1;
					}else if(!p.quote && c === ' '){
						p.a.push('');
	...
	for(var j=0; j<levels.length; j++){
		if(j>0) returnString+=" "
		if(levels[j].indexOf(" ")>0) levels[j] = '"' + levels[j] + '"'
		returnString+= ""+(counts[j]>1?counts[j]+times:"")+levels[j].replaceAll(" ","%20")
	}
	return returnString;
}
...
function getTinyURL(myURL){
	...
	var data = '{"url":"'+myURL+'","domain":"tinyurl.com"}';
	//console.log(data)
	xhr.send(data);	
...
function generateSampleStatsTable(nSamples, columns, differences=false, dataInColumns=false){
	...
	var labels=["Statistic",
				...
				"Maximum = ",
				"<a href='https://en.wikipedia.org/wiki/Quantile#Estimating_quantiles_from_a_sample'>Percentile</a> <select id='percMethod'  onchange='percentile("+nSamples+(dataInColumns?",true":"")+(differences?");getDiffPercentile(":"")+")'><option value=1>R1</option><option value=2>R2</option><option value=3>R3</option><option value=4>R4</option><option value=5>R5</option><option value=6>R6</option><option value=7>R7</option><option value=8 selected>R8</option><option value=9>R9</option></select><br><input name='perc' type='number' class='smallNumber' id='perc' value='.05' step='.01' min='0.0001' max='0.9999' onchange='percentile("+nSamples+(dataInColumns?",true":"")+(differences?");getDiffPercentile(":"")+")' /> = ",
				"Skew = ",
				"Kurtosis  = "]
	var ids=[	"statsName",
				"",
...
var menuLinks=[ ['data.html',"Datasets",false],
				[null,null,false],
				['quant1var.html?K=1',"Quantitative 1-Sample",true],
				['quant1var.html?K=2',"Quantitative 2-Sample",true],
				['quant1var.html?K=3',"Quantitative N-Sample (3+ Independent)",true],
				['dependentSamples.html',"2 Dependent (Paired) Samples",true],
				['multipleRegression.html',"Multiple Regression",true],
				['timeSeries.html',"Time Series Analysis", true],
				['survival.html',"Survival Analysis",true],
				['spatial.html',"Spatial Data Analysis",true],
				[null,null,false],
				['descriptiveStatsQualitative.html',"Qualitative 1 Variable",true],
				['contingencyTable.html',"Qualitative 2 Variable",true],
				['goodnessOfFit.html',"Goodness of Fit Test",true],
				[null,null,false],
				['bayes.html',"Bayes Theorem",true],
				['venn.html', "Venn Diagram Tool", true],
				['sampleSize.html',"Sample Size / Power Calculators",true]]				
var distDiscreteLinks = [['distributionCustom.html',"Custom Discrete",true],
				['distributionDiscrete.html?f=uniform',"Uniform",true],
				['distributionDiscrete.html?f=binomial',"Binomial",true],
				['distributionDiscrete.html?f=geometric',"Geometric",true],
				['distributionDiscrete.html?f=Poisson',"Poisson",true],
				['distributionDiscrete.html?f=hypergeometric',"Hypergeometric",true],
				['distributionDiscrete.html?f=negativebinomial',"Negative binomial",true]]
var distContinuousLinks =[['distributionCustomContinuous.html',"Custom Continuous",true],
				['distributionContinuous.html?f=uniform',"Uniform",true],
				['distributionContinuous.html?f=Gaussian',"Gaussian (normal)",true],
				['distributionContinuous.html?f=T',"Student&rsquo;s t",true],
				['distributionContinuous.html?f=Gamma',"Gamma",true],
				['distributionContinuous.html?f=exponential',"Exponetial",true],
				['distributionContinuous.html?f=chiSq',"Chi Squared",true],
				['distributionContinuous.html?f=F',"F",true],
				['distributionContinuous.html?f=Beta',"Beta",true],
				['distributionContinuous.html?f=log-normal',"Log-Normal",true],
				['distributionContinuous.html?f=Weibull',"Weibull",true],
				['distributionContinuous.html?f=Pareto',"Pareto",true],
				['distributionContinuous.html?f=Tukey',"Studentized Range",true],
				[null,null,false],
				['empiricalRule.html',"Empirical Rule Calculator",true],
				['pixelNormal.html',"Pixel Normal Calculator",true]]
var distSamplingLinks = [['samplingDistribution.html',"Sampling Distribution (Mean)",true],
				['samplingDistributionSum.html',"Sampling Distribution (Sum)",true],
				['samplingDistributionProportion.html',"Sampling Distribution (Proportion)",true],
				[null,null,false],
				['populationGenerator.html',"Population Sampling Simulator",false]]
...
"<div id=\"menu1\" class=\"menuOptions\"> "
	for(var i=0; i<menuLinks.length; i++){
		...
		if(menuLinks[i][0]==null) {menuString +="<hr>"}
		else {menuString +="<div><a href='"+menuLinks[i][0]+"' class='navlink'>"+menuLinks[i][1]+"</a>"
			if(menuLinks[i][2]) menuString +="<a class='popupButton' onClick='openData(\""+menuLinks[i][0]+"\",[],[],\""+menuLinks[i][1]+"\");'> </a>"
			menuString +="</div>"
		}
	}
menuString +="</div>"+ 
...
"<div id=\"menu2\" class=\"menuOptions\"> "
	for(var i=0; i<distDiscreteLinks.length; i++){
		if(distDiscreteLinks[i][0]==null) {menuString +="<hr>"}
		else {menuString +="<div><a href='"+distDiscreteLinks[i][0]+"' class='navlink'>"+distDiscreteLinks[i][1]+"</a>"
			if(distDiscreteLinks[i][2]) menuString +="<a class='popupButton' onClick='openData(\""+distDiscreteLinks[i][0]+"\",[],[],\""+distDiscreteLinks[i][1]+"\");'> </a>"
			menuString +="</div>"
		}
	}
menuString +="</div>"+ 
...
"<div id=\"menu3\" class=\"menuOptions\"> " 
	for(var i=0; i<distContinuousLinks.length; i++){
		if(distContinuousLinks[i][0]==null) {menuString +="<hr>"}
		else {menuString +="<div><a href='"+distContinuousLinks[i][0]+"' class='navlink'>"+distContinuousLinks[i][1]+"</a>"
			if(distContinuousLinks[i][2]) menuString +="<a class='popupButton' onClick='openData(\""+distContinuousLinks[i][0]+"\",[],[],\""+distContinuousLinks[i][1]+"\");'> </a>"
			menuString +="</div>"
		}
	}
menuString +="</div>"+ 
...
"<div id=\"menu4\" class=\"menuOptions\"> "
	for(var i=0; i<distSamplingLinks.length; i++){
		if(distSamplingLinks[i][0]==null) {menuString +="<hr>"}
		else {menuString +="<div><a href='"+distSamplingLinks[i][0]+"' class='navlink'>"+distSamplingLinks[i][1]+"</a>"
			if(distSamplingLinks[i][2]) menuString +="<a class='popupButton' onClick='openData(\""+distSamplingLinks[i][0]+"\",[],[],\""+distSamplingLinks[i][1]+"\");'> </a>"
			menuString +="</div>"
		}
	}
menuString +="</div>"+ 
...
"		<div id=\"menu5\" class=\"menuOptions\" onmouseleave=\"this.style.display='none';\"> "+ 
...
function selectColor(elmt, cIndex){
	...
	for(var i=0; i<colors.length; i++){
		...
		if(i%swatchColumns==6) contentHTML+="<br>"
	}
...
function estimatePercentile(x, p=.95, method=8){
	if(exists('percMethod')) method=parseInt(document.getElementById('percMethod').value);
//	console.log(method)		
//	console.log("Getting "+p)
	...
	switch(method){
		case 1: h=n*p; break;
		case 2: h=n*p+1/2; break;
		case 3: h=n*p-1/2; break;
		case 4: h=n*p; break;
		case 5: h=n*p+1/2; break;
		case 6: h=(n+1)*p; break;
		case 7: h=(n-1)*p + 1; break;
		case 9: h=(n+1/4)*p+3/8; break;
	}
	h--; // -1 because indexing starts at 0 not 1.
	var hFloor = Math.floor(h)
	var hCeil = Math.ceil(h)
	if(hCeil==n) { 			return(x[n-1])
	} else if(hFloor < 0){ 	return x[0]
	} else {
		if(method==1) return x[hCeil]
		if(method==2) return (x[Math.ceil(h-.5)]+x[Math.floor(h+.5)])/2
		if(method==3) return x[Math.round(h)]
	return (x[hFloor] + (h-hFloor)*(x[hCeil]-x[hFloor]))
	}
}
...
function sortArrayBy(arr, sortArr){
	return (arr.sort(function(a, b){return sortArr.indexOf(a)-sortArr.indexOf(b)}))
}
...
function cleanDataColumns(elementID, cleanSpaces=true, cleanDashes=true){
...
//		console.log(data[i].split('\t'))
		if(cleanDashes) data[i]=data[i].replaceAll('–','-')
		if(cleanDashes) data[i]=data[i].replaceAll('—','-')
...
function copyTo(toID){
	if(exists(toID)) copyVal(event.target.id, toID)
}
...
function normality(nDataSets, inCol, skipVars = 0, alsoResids=false, gof=0, alsoDiffs=false,whichResid='raw') {
	document.getElementById("normality").innerHTML=""
	var outputString="<table class='normalityTable'>"
	var first = true
	for(var dataset = 1; dataset <= nDataSets+(alsoResids?1:0)+(alsoDiffs?1:0); dataset++){
//		console.log("residuals? "+alsoResids)
		...
		if(alsoResids && dataset == nDataSets+1){
			data = Resid
			if(whichResid=='int') data=residualsInt
			if(whichResid=='ext') data=residualsExt
			setName="Residuals ("+whichResid+")"
		} else if (alsoDiffs && dataset == nDataSets+2){
			data=diffArray
			setName="Differences"
		}else if (inCol==true) {
			data = getColumn(dataset)
			setName = document.getElementById("name"+dataset).value
		} else {
...
//		console.log(dataset+ ": "+data)
		if(skipVars == 0 || skipVars!=0 && !skipVars[dataset]){
			...
			if(n>4){
				...
				var xBar = jStat.mean(data.slice())
				var s = jStat.stdev(data.slice(),true)
//				console.log(xBar)
//				console.log(s)

				outputString+="<tr><td colspan=2>Data Set: <strong>"+setName + "</strong></td></tr>\n";
				if(n>4 && n<5000){
...
if(debug)		console.log("N="+N+" K="+p.length)

//		console.log(exactMultiProb(O,p,N,true))
	var pObs = exactMultiProb(O,p,N)
	if( permutations<= 1000000  && N <= 100){
...
//		var empiricalP = moreRare/denom
		outputString +="Number of permutations: "+Math.round(permutations)+"<br>"
		outputString +="P<sub>Obs</sub>: "+fixed(pObs)+"<br>"
		outputString +="<i>p</i>-value: "+fixed(empiricalP/sumPs)+"<br>"
	} else {
		outputString +="<i>Sample size too large for exact multinomial test</i><br>"
		outputString +="Randomized Multinomial Test:<br>"
		var idxList = []
		for(var i=1; i<N-1;i++){ idxList.push(i)}
		var sumPs=0
		var empiricalP=0
		var nRand=1000000
		for(var i=0; i<nRand; i++){
			// random permutations
			var bars=[0,N]
			for(var k=0; k<K-1; k++) bars.push(Math.floor(jStat.uniform.sample(0,N)))
			bars.sort(function(a, b){return a-b})
			var x=[]
			for(var k=1; k<bars.length; k++){
				x.push(bars[k]-bars[k-1])
			}
//			if(debug) console.log(x)
		...
		outputString +="Number of randomizations: "+nRand+"<br>"
		outputString +="P<sub>Obs</sub>: "+fixed(pObs)+"<br>"
		outputString +="Approx <i>p</i>-value: "+fixed(empiricalP/sumPs)+"<br>"
...
function multinomial(counts) {
  ...
  let result = jStat.factorial(n);
  for (const count of counts) {
    result /= jStat.factorial(count);
  }
  return result;
}
...
function exactMultiProb(x,p,N){
	var pObs = 1.0
	for(var i=0; i<x.length; i++){
		pObs *= Math.pow(p[i],x[i])
	}
	return (multinomial(x) * pObs)
}
...
function bringToFront(){
	console.log(this + " at " + this.style.zIndex + "bring to front")
	if(this.style.zIndex < window.parent.topLevel){
		window.parent.topLevel++;
		this.style.zIndex = window.parent.topLevel
	}
}

var iFrameCounter = 0
var isPopup = false;
var addPopupOptions = true
var m_pos;
...
function openData(page, target=[], dataSource=[], title="",fromString=false){
	var preloadData = []
	var targets = []
	//targets is either a single string or an array of strings, ids of elements
	//dataSource is either an ID, a string of IDs or an array of data
	//fromString means that the data is literally kept in the dataSource entries
	...
	//At this point targets is an array of IDs and preloadData is an array of data strings

	if(isPopup) {
		window.parent.openData(page,targets,preloadData,title,true);
		return true;
	}
...
//	console.log(target)
	...
	var popupHeader = window.parent.document.createElement('div');
	...
	var popupClose = window.parent.document.createElement('div');
	popupClose.className='closer'
	popupClose.innerText="\u2716"
	popupClose.onclick=function(){closeMe(popup);};
	popupHeader.appendChild(popupClose)
	var popupMin = window.parent.document.createElement('div');
	var popupWin = window.parent.document.createElement('div');
	var popupMax = window.parent.document.createElement('div');
	var popupOptions = window.parent.document.createElement('div');
	popupOptions.className='opt'
	popupOptions.innerText='\u{2699}'
	...
	newFrame.src=page
//	newFrame.contentDocument.body.addEventListener('focus', function(event){
//		console.log("focus");
//	}, false);
	
	newFrame.id='iframe'+window.parent.iFrameCounter
	popup.appendChild(newFrame)
	popupOptions.onclick=function(){toggleOptions(newFrame.id);};
	
	var popupFooter = window.parent.document.createElement('div');
	popupFooter.className='expandableFooter';
	popupFooter.style.backgroundColor=colorHex
	popup.appendChild(popupFooter)
	if(addPopupOptions)popupHeader.appendChild(popupOptions)
	window.parent.iFrameCounter++
	

	
	window.parent.document.body.appendChild(popup)
	dragElement(popupHeader,"move");
	dragElement(popupFooter,"resize");

	window.parent.topLevel++;
	popup.style.zIndex = window.parent.topLevel
//	popup.addEventListener("click", bringToFront);
	
//	console.log(targets)
	if(typeof debug !== 'undefined' && debug)console.log(preloadData)
	
	newFrame.addEventListener("load", function() {
		this.contentWindow.addEventListener('focus', function(event){
			window.parent.topLevel++;
			popup.style.zIndex = window.parent.topLevel
		}, false);
		this.contentWindow.addEventListener('scroll', function(event){
			window.parent.topLevel++;
			popup.style.zIndex = window.parent.topLevel
		}, false);
		...
		var fragment = this.contentWindow.document.createDocumentFragment();
		fragment.appendChild(this.contentWindow.document.getElementById('menu5'));
		this.contentWindow.document.getElementById('container').prepend(fragment);
		this.contentWindow.document.getElementById('menu5').style.top='0px';
		this.contentWindow.document.getElementById('menu5').style.position='fixed';
		this.contentWindow.document.getElementById('container').style.position='relative';
		this.contentWindow.document.getElementById('menu5').style.zIndex='10';
		...
		if (page=="multipleRegression.html" && targets.length>0){
		...
		if(page=="descriptiveStatsQualitative.html"){
			this.contentWindow.dataInputChanged()
		}
		...
		if(page=="contingencyTable.html"){
			...
			this.contentWindow.sortRowCol(true,true);
		}
...
//			console.log("response var: "+preloadData[preloadData.length-1])
		...
		this.contentWindow.isPopup=true;
	});
}

function toggleOptions(iFrameID){
	console.log('toggleOptions '+iFrameID)
	if(document.getElementById(iFrameID).contentWindow.document.getElementById('menu5').style.display=='block'){
		document.getElementById(iFrameID).contentWindow.document.getElementById('menu5').style.display="none"
	}else{
		document.getElementById(iFrameID).contentWindow.document.getElementById('menu5').style.display="block"
		document.getElementById(iFrameID).contentWindow.scrollTo(0, 0);
	}
}
...
function dragElement(elmnt,type="move") {
  var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
  var hgt = elmnt.parentElement.style.height;
  ...
  function dragMouseDown(e) {
	...
	window.parent.topLevel++;
	elmnt.parentElement.style.zIndex = window.parent.topLevel
	elmnt.parentElement.classList.add("moving");
  }

  function elementDrag(e) {
	if(elmnt.parentElement.classList.contains('maxFloater'))return false;
	e = e || window.event;
	e.preventDefault();
	...
	if(type=="move"){
		...
		if((elmnt.parentElement.offsetTop - pos2)<0) elmnt.parentElement.style.top="0px";
		if((elmnt.parentElement.offsetLeft - pos1) <0) elmnt.parentElement.style.left="0px";
	}
	if(type=="resize"){
		elmnt.parentElement.style.height = Math.max(30,(e.clientY-elmnt.parentElement.offsetTop)) + "px";
	}
  }
  ...
  function closeDragElement() {
	...
	elmnt.parentElement.classList.remove("moving");
  }
}
  var topLevel = 10
...
var sampleStatsNames=["Means","Medians","Std. Dev.","Variance","Q1","Q3","Sum", "Max","Min","IQR","Bootstrap T"]
var sampleStatsVals = [1,2,3,6,4,5,7,8,9,10,11]
var sampleStatsRequire=[null,null,null,null,null,null,null,null,null,null,"data1"]
...
function recalcSampleStat(){
	...
	var xbar = exists("data1")?jStat.mean(parseData(1)):0
	for(var i=0; i<samples.length; i++){
		...
		if(sampleStatID==11) sampleStat = fixed((jStat.mean(data)-xbar)/(jStat.stdev(data,true)/Math.sqrt(data.length)))
...
function buildContent(panelid,options=[]){
	...
	if(panelid=="sampling"){
		...
		for(var i=0; i<sampleStatsNames.length; i++){
			if(sampleStatsRequire[i]==null || exists(sampleStatsRequire[i]))contentString+="			<option value="+sampleStatsVals[i]+" "+(i==0?" selected":"")+">"+sampleStatsNames[i]+"</option>\n"
		}
		contentString+="			</select>"
		contentString+="		</td></tr>"
		...
		contentString+="		<tr><td></td><td><button type='button' value='analyze' class='analyzeButton' onClick='openData(\"quant1var.html\",\"data1\",\"sampleMeans\",nameSampleMeansWindow());'>analyze</button></td></tr>"
		contentString+="	</table>"
		contentString+="</div>"
	}
	contentString +="	</div>"
...
function setValue(id,val){
	if(exists(id)) document.getElementById(id).value=val
}
