...
function refreshNormalityPlot(data, forceIt=false, canvas='qqPlot', dataName=document.getElementById('name1').value, varUnits=document.getElementById('varUnits1').value, varName=exists('varName1')?document.getElementById('varName1').value:"", forceBands = false){
	if (!forceIt && !isOpen("headerNormality")) return false;
	var drawBands = (exists("qqbands") && document.getElementById('qqbands').checked) || forceBands
	...
	for(var i=0; i<n; i++){
		...
		ctx.beginPath();
		ctx.arc(tx(qX),ty(pY), dotRadius*scale, 0, 2 * Math.PI);
		ctx.fill();
		if(n<100) ctx.stroke();
	}
...
function drawEmpiricalLines(ctx,a,b){
	...
	var scale=w/490
	var scale=w/490
	var lMargin=50*scale, rMargin=30*scale, tMargin=20*scale, bMargin=30*scale
...
//		dp = by*d+Ay
		ctx.beginPath();
		ctx.setLineDash([5,0]);
		ctx.moveTo(tx(xx),ty(0))
		ctx.lineTo(tx(xx),ty(d)) 
		ctx.stroke();
		if(i!=0){
			ctx.beginPath();
			ctx.setLineDash([5, 3]);
			ctx.moveTo(tx(xx),ty(d))
			ctx.lineTo(tx(xx),(55+Math.abs(i)*20)*scale)
			ctx.stroke()
		}
	}
	...
	ctx.setLineDash([5,0]);
	for(var i=0; i<percs.length; i++){
		drawHorizArrow(ctx,tx(a+b*(-i-1)), tx(a+b*(i+1)),(75+i*20)*scale,4*scale)
		ctx.beginPath()
		ctx.fillStyle="white"
...
function drawDensity(a,b,lFill,hFill, tail, shift=0, vscale=1, skipTicks=false, canvasID="densityFunction",renderPDF=true, nTicksOverride=0) {
...
//	console.log(maxDensity)
	if(debug)console.log(maxDensity)
	var lowX =  Xbounds[0]
	var highX = Xbounds[1]
		
	setChartParams(lowX,highX,0, maxDensity,chartWidth,chartHeight,lMargin,tMargin)
	var steps = (distribution=='Tukey'?100:500)
	var inc = (highX-lowX)/steps
...
if(renderPDF){
	...
	lFill = Math.max(lowX,lFill)
	hFill = Math.min(highX,hFill)
	var fillLB = [lFill], fillUB=[hFill]
	if(tail){ fillLB=[lowX,hFill], fillUB=[lFill, highX]}
	
	for(var k=0; k<fillLB.length; k++){
		ctx.beginPath();
		ctx.moveTo(tx(fillLB[k]),ty(0)-1)
		for(var i=fillLB[k]; i<=fillUB[k]; i+=inc){
			d =  Math.min(maxDensity,density(i,a,b))
			ctx.lineTo(tx(i), ty(d))
		}
		ctx.lineTo(tx(fillUB[k]), ty(density(fillUB[k],a,b)))
		ctx.lineTo(tx(fillUB[k]), ty(0)-1)
		ctx.fill();
	}
	//console.log(savedDensity)
	//Next Draw line
	ctx.beginPath();
	...
	ctx.stroke();
	chartAxisY(ctx,lMargin,tMargin+chartHeight,tMargin,0,maxDensity,8,"Density",false,lMargin+chartWidth, false, fontSizeAxis*scale)
} else {
...
//		dp= by*d+Ay
	...
	if(nTicksOverride>0)nTicks=nTicksOverride
...
//			alert(Xshift)
	...
	var Xbounds=[]
	var probDensity=[]
	var xEmpBins=Math.min(200, Math.floor(Math.sqrt(a.length)))
...
function histogramPlot(nSamples, canvasID='histogram'){
	...
	var relative=nSamples>1
	if(exists("relativeFreqHist")) relative=document.getElementById("relativeFreqHist").checked
...
//	console.log(maxFreq)
	...
	if(!discrete){
		axisXlbl1 = jStat.min(allData)-h/2*0
		axisXlbl2 = jStat.max(allData)+(cumulative?-1:1)*h/2*0
	...
	var grpLabels=[]
	var normalCurve=false
	if(exists('overlayNormal') && document.getElementById('overlayNormal').checked) normalCurve=true
	...
	for(var dataset=0; dataset<nSamples; dataset++){
		...
		var cHex = colorHexes[labelColors[dataset]]
	...
	//		console.log(curveScale)
			ctx.beginPath()
			for(i=0; i<=50; i++){
				x=(jStat.min(allData)+i/50*(jStat.range(allData))-mu)/s
				y=jStat.normal.pdf(x,0,1)/curveScale
				if(i==0) ctx.moveTo(lMargin+i/50*chartWidth,tMargin-y*(chartHeight-gap*(nSamples-1))/nSamples+chart_plus_gap*(dataset+1)-gap)
				if(i>0) ctx.lineTo(lMargin+i/50*chartWidth,tMargin-y*(chartHeight-gap*(nSamples-1))/nSamples+chart_plus_gap*(dataset+1)-gap)
			}
			ctx.stroke()
		}
...
//	chartAxisY(ctx,lMargin,tMargin + (gap+(chartHeight-gap*(nSamples-1))/nSamples) * (dataset+1)-gap,tMargin + (gap+(chartHeight-gap*(nSamples-1))/nSamples) * dataset, 0 ,top ,nTicksY,,true,lMargin+chartWidth)
	...
	chartTitle(ctx,((grpLabels.length)==1?(grpLabels[0]+" "):"")+(cumulative?"Cumulative Relative ":"")+(relative?"Relative ":"")+"Frequency Histogram", lMargin+.5*chartWidth, 0,fontSizeTitle*scale);
	...
	if((grpLabels.length)>1) chartGroupLabels(ctx, grpLabels,lMargin+5,tMargin+10*scale,chart_plus_gap,true,fontSizeAxis*scale)
}
...
//		data=[]
//		for(var i=0; i<dataCopy.length; i+=Math.ceil(dataCopy.length/1000)) data.push(dataCopy[i])
		var dataMin = jStat.min(data)-bandwidth*hBuffer
		var dataMax = jStat.max(data)+bandwidth*hBuffer
		var f = density1d(data, {bandwidth:bwdth, extent:[dataMin,dataMax]});
		f=f.grid()
		var maxF = jStat.max(f);
		step = (dataMax-dataMin)/f.length
...
//		}
		...
		for(var i=0; i<5;i++){
			...
			if(i==2){ctx.lineWidth=2*scale;}else{ctx.lineWidth=1*scale;}
			ctx.beginPath();
...
function densityPlot(nSamples=1, canvasElmt="densityPlot", dataName=document.getElementById('name1').value, varUnits=document.getElementById('varUnits1').value, varName=exists('varName1')?document.getElementById('varName1').value:""){
	...
	for(var dataSetNum=1; dataSetNum<=nSamples; dataSetNum++){
		...
		var cHex = colorHexes[labelColors[dataSetNum-1]]
		//Set color of Fill
		if (document.getElementById("colorful").checked==true)
			ctx.fillStyle = opaque?cHex:"rgba("+hexToRgb(cHex).r +","+hexToRgb(cHex).g +","+hexToRgb(cHex).b +",.3)";
		else
			ctx.fillStyle = "#C0C0C0";
...
function barchart(){
	...
	refreshFreqTable()
	var n=nData;
	var labels = Object.keys(freqTable)
	var frequencies = Object.values(freqTable)
...
//	var nBars = labelOrder.length				
	...
	for(var i=0; i<labelOrder.length;i++){
		var count=freqTable[labelOrder[i]];
//		for(j=0; j<data.length; j++){
...
//		}
	...
	var gridRes = +document.getElementById('spatialRes').value*scale
	var n=data[0].length
	...
	//				console.log(i+" "+j+ " "+v)
			...
			if(debug)console.log("variogram ="+fitModel)
			if(debug)console.log("nugget ="+fitModel.nugget)
			if(debug)console.log("range ="+fitModel.range)
			if(debug)console.log("sill ="+fitModel.sill)
		...
		//	console.log("quadratic trend")
			...
			//console.log(X)
			var Y=data[2].slice()
			//console.log(numeric.dot(numeric.transpose(X),X))
...
//		console.log(sites)
		for(var i=0; i<diagram.cells.length; i++){
			...
			color=(data.length<3?"white":colorRamp(cRamp, ((data[2][datai]-vRng[0])/(vRng[1]-vRng[0]))))
...
//		console.log(cRamp)
		...
		var nTicks = (data.length<3?9:Math.min(uniqueValues(data[2]).length,9))
...
function scatterplot(Xdata,Ydata,logisticReg=false, xLabel="", yLabel="", drawLine=false, CIband=false, predBand=false, groups=false, drawLines=false, x=[], y=[],legend=[],xVals=[], canvas='scatterplot',  units=["",""], pointLabels=[], overrideTitle='') {
//	console.log("scatterplot: "+xVals)

	var categoricalX = (xVals.length>0 && !isNumeric(xVals[0]))
	
	var c=document.getElementById(canvas)
	var ctx=c.getContext("2d")
	//ctx.font = "12px sans-serif";0
	ctx.save();
	ctx.fillStyle = "white";
	ctx.fillRect(0, 0, c.width, c.height);	
	var scale = c.width/480
	...
	var xMargin=(categoricalX?(.5/xVals.length*(chartWidth-2*cMargin)):0)
	setChartParams(Xmin, Xmax, Ymin, Ymax,chartWidth-2*cMargin-2*xMargin,chartHeight-2*cMargin, lMargin+cMargin+xMargin, tMargin+cMargin)
...
//console.log("legend length "+ legend.length)
	if(drawLines && x.length>0){
		for(var k=0; k<Math.max(1,legend.length); k++){
			ctx.beginPath()
...
//	console.log(pointLabels)
	if(categoricalX && exists("smartPlot") && document.getElementById("smartPlot").checked){
		var nBoxes = +jStat.max(Xdata)-jStat.min(Xdata)+1
		var boxData = new Array
//		console.log(Xdata)
		for(var j=0; j<nBoxes; j++) boxData[j]=[]
		for(var i=0; i<Ydata.length; i++){
			boxData[Xdata[i]].push(Ydata[i])
		}
		for(var i=0; i<nBoxes; i++){
			var x1 = lMargin+cMargin
			var x2 = x1+chartWidth-2*cMargin
			var y1 = tMargin+cMargin
			var y2 = y1 + chartHeight-2*cMargin
			var boxplotData = boxifyData(boxData[i])
			
			var bounds=[x1+(i)*(x2-x1)/nBoxes+5*scale,
						y2,
						(x2-x1)/nBoxes-10*scale,
						y2-y1]
			ctx.fillStyle="rgba(0,0,0,.2)";
			ctx.lineWidth=1.5*scale
			drawBoxPlot(ctx, bounds, boxplotData, [Ymin,Ymax],dotRadius,scale)
		}
	...
	//		var qY = (-Ymin + Ydata[i])/qYrange
			...
			ctx.arc(tx(Xdata[i]),ty(Ydata[i]), dotRadius*scale, 0, 2 * Math.PI);
			ctx.fill();
			ctx.stroke();
			if(pointLabels.length>i && pointLabels[i]!=''){
				ctx.beginPath();
				ctx.fillStyle = "black";
				var yadj = (ty(Ydata[i])<50*scale ? 16:-5)*scale
				ctx.fillText(pointLabels[i], tx(Xdata[i]),ty(Ydata[i])+yadj);
			}
		}
...
//		console.log("using x values!")
		chartAxisX(ctx,tMargin + chartHeight,lMargin+cMargin+xMargin,lMargin+chartWidth-cMargin-xMargin,Xmin,Xmax,Math.min(xVals.length-1,10),xLabel+(units[0]!=""?" ("+units[0]+")":""),false,xVals,fontSizeAxis*scale)
	}
	...
	chartTitle(ctx, overrideTitle==''?"Scatter Plot":overrideTitle, lMargin+.5*chartWidth, 0, fontSizeTitle*scale)
}
...
//   RESIDUAL PLOT
//----------------------------------------------------------
function residualplot(Xdata, xLabel="", absValue=false, xLabels=[],Xvar=1,sqrtValue=false, canvas="residualplot",residType=0, forceChartTitle="") {
	var c=document.getElementById(canvas)
	var ctx=c.getContext("2d")
	ctx.font = "12px sans-serif";
	ctx.save();
	...
	var prefix2=''
//	if(){
//		console.log(document.getElementById('residWhich').value)
		if(exists('residWhich')&&document.getElementById('residWhich').value=='int' || residType==1){
			resids=residualsInt.slice()
			prefix2='Internally Studentized '
		}
		if(exists('residWhich')&&document.getElementById('residWhich').value=='ext' || residType==2){
			resids=residualsExt.slice()
			prefix2='Externally Studentized '

//		}		
	}
	
	
	if(absValue || sqrtValue){
		for(var i=0; i<resids.length; i++) {
			resids[i]=Math.abs(resids[i])
			if(sqrtValue) resids[i]=Math.sqrt(resids[i])
		}
	}
	var Xmin = jStat.min(Xdata)
	var Xmax = jStat.max(Xdata)
...
//	dotRadius *= var dotRadius=(n<100?2:4)

	for(var i=0; i<n; i++){
		var X = (-Xmin + Xdata[i])/Xrange
		var Y = (-Ymin + resids[i])/Yrange
		ctx.beginPath();
		ctx.arc(lMargin+5+xMargin+(X)*(chartWidth-2*xMargin-10),tMargin+5+(1-Y)*(chartHeight-10), dotRadius*scale, 0, 2 * Math.PI);
		ctx.fill();
...
*/	
	...
	if(xLabels.length==0){
		chartAxisX(ctx,tMargin + chartHeight,lMargin+5,lMargin+chartWidth-5,Xmin,Xmax,10,xLabel+(units[0]!=""?" ("+units[0]+")":""), false,[],fontSizeAxis*scale)
	} else {
//		console.log("using x values!")
		chartAxisX(ctx,tMargin + chartHeight,lMargin+5+chartWidth/(xLabels.length*2),lMargin-5+(1-1/(xLabels.length*2))*chartWidth,Xmin,Xmax,xLabels.length-1,xLabel+(units[0]!=""?" ("+units[0]+")":""),false,xLabels,fontSizeAxis*scale)
	}
	prefix=""
	if(absValue) prefix="Absolute "
	if(sqrtValue) prefix="Root "
	chartAxisY(ctx,lMargin,tMargin+chartHeight-5,tMargin+5,Ymin,Ymax,10,prefix+prefix2+"Residual"+(units[1]!=""?" ("+units[1]+")":""), false,chartWidth,true, fontSizeAxis*scale)	
	chartTitle(ctx, forceChartTitle==""?"Residual Plot":forceChartTitle, lMargin+.5*chartWidth, 0, fontSizeTitle*scale)
