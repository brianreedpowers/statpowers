...
function krigLoss(X){
	...
	var sill = X[1]
	var range = X[3]	
	var model = (exists('krigingModel')?document.getElementById('krigingModel').value:"spherical")
	var sumsq = 0
	for(var i=0; i<lag.length; i++){
		sumsq += Math.pow(semi[i] - semivar(lag[i],X, model),2)
	}
	return sumsq
}
...
var kriging = function() {
    ...
    kriging_variogram_gaussian = function(h, nugget, range, sill, A) {
		return nugget + (sill-nugget)*( 1.0 - Math.exp(-h*h/(range*range/3.0)) );
    };

    kriging_variogram_exponential = function(h, nugget, range, sill, A) {
		return nugget + (sill-nugget)*( 1.0 - Math.exp(-(1.0/A) * (h/range)) );
    };
    kriging_variogram_spherical = function(h, nugget, range, sill, A) {
//		if(h>range) return nugget + (sill-nugget);
		return nugget + ((sill-nugget))*(h>range?1: 1.5*(h/range) - 0.5*Math.pow(h/range, 3) );
    };
	...
	for(i=0,k=0;i<n;i++)
	    for(j=0;j<i;j++,k++) {
		dist[k] = Array(2);
		dist[k][0] = distance(x[i],y[i],x[j],y[j])
		//Math.pow(
		//    Math.pow(x[i]-x[j], 2)+
		//    Math.pow(y[i]-y[j], 2), 0.5);
		dist[k][1] = Math.abs(t[i]-t[j]);
	    }
	dist.sort(function(a, b) { return a[0] - b[0]; });
	variogram.range = dist[(n*n-n)/2-1][0];

	// Bin lag distance
	var lags = Math.min(15,(n*n-n)/2);
...
//		console.log(variogram.range/3)
//		console.log(jStat.max(semi))
	
//		var solution = fmin.nelderMead(krigLoss, [0,jStat.max(semi),variogram.range/3])			
//		console.log(solution)
//		solution.x[1]=Math.min(jStat.max(semi),solution.x[1])
//		solution.x[2]=Math.min(variogram.range/3,solution.x[2])
...
*/
	...
	variogram.range = solution.x[2]
	variogram.nugget = solution.x[0];
	variogram.sill = solution.x[1]+variogram.nugget;
	...
	for(i=0;i<n;i++) {
	    for(j=0;j<i;j++) {
			K[i*n+j] = variogram.model(distance(x[i],y[i],x[j],y[j])/*Math.pow(Math.pow(x[i]-x[j], 2)+
						    Math.pow(y[i]-y[j], 2), 0.5)*/,
					   variogram.nugget,
					   variogram.range,
					   variogram.sill,
	...
	for(i=0;i<variogram.n;i++)
	    k[i] = variogram.model(distance(variogram.x[i],variogram.y[i],x,y)/*Math.pow(Math.pow(x-variogram.x[i], 2)+
	...
	return variogram.model(0, variogram.nugget, variogram.range, variogram.sill, variogram.A)+
			kriging_matrix_multiply(kriging_matrix_multiply(k, variogram.K,	1, variogram.n, variogram.n),
				k, 1, variogram.n, 1)[0];
    };
