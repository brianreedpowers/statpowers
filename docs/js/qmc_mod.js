   ...
   var solutionFound = true;
    ...
    if (solutionFound) {
      ...
      for (var i = 0; i < this.minimalTermPrims.length; i++) {
		  var addParens=(this.minimalTermPrims.length>1 && this.minimalTermPrims[i].termString.length>2)
        if (!firstL) {
          this.minimalTerm += " &cup; ";
          this.coloredMinimalTerm += " &cup; ";
        }
        this.minimalTerm += (addParens?"(":"")+this.minimalTermPrims[i].termString+(addParens?")":"");
        this.coloredMinimalTerm += (addParens?"(":"")+this.minimalTermPrims[i].coloredTermString+(addParens?")":"");
        firstL = false;
      }
...
/*    if(myDiv === -1) return;
  ...
  */  
    ...
    var termDiv = document.getElementById('displaySetNotation')
	var outputString = this.data.minimalTerm
//	console.log(outputString)
	if(outputString=="1") outputString = "S"
	if(outputString=="0") outputString =""
    termDiv.innerHTML = outputString;
//    myInnerDiv.appendChild(termDiv);
//f    myDiv.appendChild(myInnerDiv);
  };
